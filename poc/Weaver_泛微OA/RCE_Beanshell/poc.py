...
class POC(POCBase):
    ...
    _info = {
        ...
        "VulnDesc" : """
            泛微e-cology OA系统的Java Beanshell接口可被未授权访问, 
            攻击者调用该Beanshell接口, 可构造特定的HTTP请求绕过泛微本身一些安全限制从而达成远程命令执行, 
            漏洞等级严重
        """,                                # 漏洞简要描述
    ...
    def _verify(self):
        ...
        """
        vuln = [False,""]
        url = self.target + "/weaver/bsh.servlet.BshServlet" # url自己按需调整
        ...
        try:
            ...
            if "BeanShell Test Servlet" in req.text and req.status_code == 200:
                vuln = [True,req.text]
            else:
                vuln = [False,req.text]
